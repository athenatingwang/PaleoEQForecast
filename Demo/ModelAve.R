## Set the Working Directory to the "Demo" folder
#setwd("/PaleoEQ/Demo")
...
p50Pois <- p100Pois <- p500Pois <- array(NA,c(nfault,K,150))
p50gam <- p100gam <- p500gam <- array(NA,c(nfault,K,150))
p50weib <- p100weib <- p500weib <- array(NA,c(nfault,K,150))
p50bpt <- p100bpt <- p500bpt <- array(NA,c(nfault,K,150))
p50lnorm <- p100lnorm <- p500lnorm <- array(NA,c(nfault,K,150))

p50modAve <- p100modAve <- p500modAve <- array(NA,c(nfault,K,150))

modAveFore <- Fore.Pois <- Fore.gam <- matrix(NA,nrow=nfault,ncol=7500)
Fore.weib <- Fore.bpt <- Fore.lnorm <- matrix(NA,nrow=nfault,ncol=7500)
...
for (faulti in (1:nfault)){
  ...
  load(paste("../ResultsDemo/PoisRes/PoisFault-mcmc",faulti,".image",sep=""))
  ...
  load(paste("../ResultsDemo/GammaRes/GammaFault-mcmc",faulti,".image",sep=""))
  ...
  load(paste("../ResultsDemo/WeibRes/WeibFault-mcmc",faulti,".image",sep=""))
  ...
  load(paste("../ResultsDemo/bptRes/bptFault-mcmc",faulti,".image",sep=""))
  load(paste("../ResultsDemo/bptRes/HPD-toccBPT",faulti,".image",sep=""))
  library(actuar)
  ...
  load(paste("../ResultsDemo/lnormRes/lnormFault-mcmc",faulti,".image",sep=""))
...
save(ave.probres,file=paste("../ResultsDemo/ModAveProbRes.image",sep=""))
save(ave.logprobres,file=paste("../ResultsDemo/ModAveLogProbRes.image",sep=""))
save(ave.foreres,file=paste("../ResultsDemo/ModAveForeRes.image",sep=""))

write.csv(ave.foreres$weights,"../ResultsDemo/WAICweights.csv")
...
save(params,file=paste("../ResultsDemo/ParamEstNew.image",sep=""))

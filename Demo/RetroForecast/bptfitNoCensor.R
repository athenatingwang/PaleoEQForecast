# Fitting a BPT renewal process without censoring to each MC sample of the earthquake chronologies 

# This file contains the jags model and general jags run for any data.
# Refer to bptfit.allfaults.R which sources this bptfitNoCensor.R and fits
# the BPT renewal process without censoring to the occurrence times from each fault segment.
...
bptfit <- function(faulti,inter.NA, N, K,
  ...
  params <- c("mu","alpha","sigmaY","sigmaZ","Y","Z")
	...
	inits <- function(){list("mu"=mu0+runif(1,-mu0/4,mu0/4),
	                         "alpha"=alpha0+runif(1,-alpha0/3,alpha0/3))
			}
	...
	save(mod.mcmc,file=paste("../../ResultsDemo/bptSimRes2/bptFault-mcmc",faulti,".image",sep=""))

...
#b <- dir("../../Results/lnormSimRes2/GelmanDiag")
#bsub <- substr(b,16,17)
...
#count <- 1
#for (j in bsub){
# temp <- read.csv(paste("../../Results/lnormSimRes2/GelmanDiag/GelmanDiaglnorm",j,".csv",sep=""))
...
## n.thin=1000 are used in the lnormfitNoCensor.R file.
...
source('../RetroForecast/lnormfitNoCensor.R')
...
while(!gelman.1.2){
  ...
  {
  tryCatch({
   modtem <- lnormfit(i,inter.NA, t.occ, N, K,n.iter.mc,n.burnin.mc,n.thin.mc)
   mod.mcmc <- modtem
   mod <- mod.mcmc
   catched = TRUE
   }, error=function(e){})
  ...
  if (temp.gelman<=gelman.max){
   ...
   save(mod.mcmc,file=paste("../../ResultsDemo/lnormSimRes2/lnormFault-mcmc",i,".image",sep=""))
   write.csv(t(gelman),file=paste("../../ResultsDemo/lnormSimRes2/GelmanDiag/GelmanDiaglnorm",i,".csv",sep=""))
   ...
   save(hpd.lnorm,file=paste("../../ResultsDemo/lnormSimRes2/HPDlnorm",i,".image",sep=""))
  }
}
...
#Time difference of 6.773811 mins

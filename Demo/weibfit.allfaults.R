# This file sources weibfit.R and fits a Weibull renewal process 
...
#### all fault segments.
#### In the paper, all diagnostics values are less than 1.02.
###########################################################################
## load Gelman Diagnostics and check which faults didn't converge

#b <- dir("../Results/WeibRes/GelmanDiag")
#bsub <- substr(b,15,16)
...
#count <- 1
#for (j in bsub){
# temp <- read.csv(paste("../Results/WeibRes/GelmanDiag/GelmanDiagWeib",j,".csv",sep=""))
...
## n.thin=1000 are used in the weibfit.R file.
...
{
 tryCatch({
   mod <- weibfit(i,inter.NA, t.occ, N, K, isCensor, CensLim,
...
write.csv(t(gelman),file=paste("../ResultsDemo/WeibRes/GelmanDiag/GelmanDiagWeib",i,".csv",sep=""))
save(hpd.weib,file=paste("../ResultsDemo/WeibRes/HPDWeib",i,".image",sep=""))
...
#Time difference of 12.37902 mins

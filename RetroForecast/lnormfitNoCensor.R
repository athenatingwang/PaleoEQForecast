# Fitting a lognormal renewal process without censoring to each MC sample of the earthquake chronologies 

# This file contains the jags model and general jags run for any data.
# Refer to lnormfit.allfaults.R which sources this lnormfitNoCensor.R and fits
# the lognormal renewal process without censoring to the occurrence times from each fault.
...
lnormfit <- function(faulti,inter.NA, t.occ, N, K){
	library(lattice)
	library(R2jags)	

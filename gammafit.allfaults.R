# This file sources gammafit.R and fits
# a gamma renewal process to the occurrence times from each fault.
...
b <- dir("/nesi/nobackup/nesi00326/GammaRes/GelmanDiag")
bsub <- substr(b,16,17)
...
count <- 1
for (j in bsub){
 temp <- read.csv(paste("/nesi/nobackup/nesi00326/GammaRes/GelmanDiag/GelmanDiagGamma",j,".csv",sep=""))
...
{
 tryCatch({
   mod <- gammafit(i,inter.NA, t.occ, N, K, isCensor, CensLim, plot.Y=T)
   catched = TRUE
   }, error=function(e){})
 print(catched)
}
...
write.csv(t(gelman),file=paste("/nesi/nobackup/nesi00326/GammaRes/GelmanDiagGamma",i,".csv",sep=""))
save(hpd.gamma,file=paste("/nesi/nobackup/nesi00326/GammaRes/HPDGamma",i,".image",sep=""))

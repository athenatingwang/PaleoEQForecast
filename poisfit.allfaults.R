# This file sources poisfit.R and fits
# a Poisson process to the occurrence times from each fault.
...
b <- dir("/nesi/nobackup/nesi00326/PoisRes/GelmanDiag")
bsub <- substr(b,15,16)
...
count <- 1
for (j in bsub){
 temp <- read.csv(paste("/nesi/nobackup/nesi00326/PoisRes/GelmanDiag/GelmanDiagPois",j,".csv",sep=""))
 gelmandiag[count,] <- as.numeric(temp[-1]) 
 gelmanmax[count] <- max(as.numeric(temp[-1]))
 count <- count+1
}

if (length(which(gelmanmax>=1.01))>=1){
  ind <- bsub[-which(gelmanmax>=1.01)]
}else {ind <- bsub}
...
{
 tryCatch({
   mod <- poisfit(i,inter.NA, t.occ, N, K, isCensor, CensLim)
   catched = TRUE
   }, error=function(e){})
 print(catched)
}
...
# Some more plots
xyplot(mod[,c(1:4,4+K)], layout=c(2,4), aspect="fill")

# Density plot
densityplot(mod[,c(1:4,4+K)], layout=c(2,4), aspect="fill")


#Auto-correlation plot
autocorr.plot(mod[,c(1:4,4+K)])
...
t.fore.mean <- as.vector(mod.mat[,4+K])
...
write.csv(t(gelman),file=paste("/nesi/nobackup/nesi00326/PoisRes/GelmanDiagPois",i,".csv",sep=""))
save(hpd.pois,file=paste("/nesi/nobackup/nesi00326/PoisRes/HPDPois",i,".image",sep=""))
